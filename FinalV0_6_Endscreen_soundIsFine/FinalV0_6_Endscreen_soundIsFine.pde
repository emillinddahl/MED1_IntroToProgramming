...
void setup() {
  ...
  for (int i=0; i< balls.length; i++) {
    balls[i] = new ball(int(random(0, 1200)), int(random(0, 800)), ballsize, speed, speed); // a for loop creating all 8 ball objects.
  }
  for (int i=0; i< hb.length; i++) {
    hb[i] = new hblock(600, int(random(50, 750)), 50, 50, 4, 4); //creating the block object.
  }
}
void draw() {
